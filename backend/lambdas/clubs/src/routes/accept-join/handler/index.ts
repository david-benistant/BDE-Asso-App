...
export const baseHandler: Handler = async (
    ...
    const joinRequest = await joinRequestRepository.get(id, event.body.userId);
    ...
    const newClub = new ClubValueObject({
        ...
        members: [
            ...club.getMembers(),
            { id: joinRequest.getUserId(), role: Roles.MEMBER, displayName: user.getDisplayName() },
        ],
    });

    await joinRequestRepository.delete(id, event.body.userId);

    const newUser = new UserValueObject({ ...user.getObject(), joinedClubs: [ ...user.getJoinedClubs(), newClub.getId()] })
